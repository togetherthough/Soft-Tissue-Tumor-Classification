...
### Problem: Gray Sagittal Views

In orthogonal visualizations, sagittal views (especially at X=80) may appear almost completely gray/padded, even though X=80 should contain anatomy.

### Root Cause: Heavy Z-axis Padding

The issue is **heavy Z-axis padding**, not missing data in the X dimension.

#### Original Volume Example (GIST-013)
- **Shape**: 512×512×**76** 
...
Modify visualization function to automatically detect non-padding regions:
...
Voxel spacing is critical for accurate volume measurements. Different scans have different physical resolutions, making voxel counts alone misleading.
...
## Spacing in Notebooks

### Updated Files

1. **`notebooks/visualization/Raw_Image_Display.ipynb`**
   - `load_raw_volume()` now returns `(volume, spacing)` tuple
   - Extracts spacing from SimpleITK metadata
   - Converts from SimpleITK (x, y, z) to numpy (z, y, x) convention

2. **`notebooks/visualization/Dataset_Analysis.ipynb`**
   - Added `tumor_volumes_mm3` and `tumor_volumes_cm3` fields
   - Computes physical volumes: `voxel_count × spacing`
   - Visualizations now show cm³ instead of voxel counts
...
- Dataset analysis: `notebooks/visualization/Dataset_Analysis.ipynb`
- Raw image display: `notebooks/visualization/Raw_Image_Display.ipynb`
