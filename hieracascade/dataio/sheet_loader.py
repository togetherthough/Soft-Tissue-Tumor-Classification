...
def load_from_sheet_csv(
    ...
    Args:
        ...
        label_col: Column name for tumor class labels (default: 'target')
    ...
    if label_col not in df.columns:
        raise ValueError(f"Column '{label_col}' not found in sheet.csv")
    ...
    for idx, row in df.iterrows():
        ...
        category = str(row[label_col]).lower().strip()
...
def create_index_from_sheet(
    ...
    **kwargs
    ...
    Args:
        ...
        output_csv: Optional path to save the processed labels CSV
    ...
    index = load_from_sheet_csv(sheet_path, data_root, **kwargs)
